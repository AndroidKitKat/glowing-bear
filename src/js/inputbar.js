            ...
            function (
                ...
                $scope.inputChanged = function () {
                    ...
                    for (var i = 0; i < segments.length; i++) {
                        ...
                        // emojify segment
                        var emojified = emojione.shortnameToUnicode(
                            segments[i]
                        );
                    ...
                    if (changed) {
                        // Only re-assemble if something changed
                ...
                $scope.completeNick = function () {
                    ...
                    var completion_suffix =
                        models.wconfig["weechat.completion.nick_completer"];
                    var add_space =
                        models.wconfig["weechat.completion.nick_add_space"];
                    var nickComp = IrcUtils.completeNick(
                        input,
                        caretPos,
                        $scope.iterCandidate,
                ...
                $scope.completeCommand = function (direction) {
                    ...
                    var cycleCompletionList = function (direction) {
                        ...
                        if (direction === "backward") {
                            ...
                            if (commandCompletionPositionInList < 0) {
                                ...
                                commandCompletionPositionInList =
                                    commandCompletionList.length - 1;
                            }
                        }

                        // Check we have not reached the end of the cycle list
                        if (
                            commandCompletionList.length <=
                            commandCompletionPositionInList
                        ) {
                        ...
                        var commandBeforeReplace = $scope.command.substring(
                            0,
                            commandCompletionPosition -
                                commandCompletionBaseWord.length
                        );
                        var commandAfterReplace = $scope.command.substring(
                            commandCompletionPosition,
                    ...
                    if (input + activeBuffer.id !== previousInput) {
                        ...
                        connection
                            .requestCompletion(activeBuffer.id, caretPos, input)
                            .then(function (completionObject) {
                                ...
                                commandCompletionAddSpace =
                                    completionObject.add_space;
                                commandCompletionBaseWord =
                                    completionObject.base_word;
                ...
                $scope.sendMessage = function () {
                    ...
                    if ($scope.command !== undefined && $scope.command !== "") {
                        ...
                        if (index >= 0) {
                            var queryName = $scope.command.substring(
                                spacepos + 1
                            );
                ...
                $rootScope.addMention = function (bufferline) {
                    ...
                    if (newValue.length > 0) {
                        ...
                        if (
                            ...
                            var lastWord = trimmedValue.slice(
                                lastSpace,
                ...
                $rootScope.handleKeyPress = function ($event) {
                    ...
                    var altg = $event.getModifierState
                    ...
                    if ($rootScope.showJumpKeys) {
                        ...
                        if (!$event.altKey && code > 47 && code < 58) {
                            ...
                            if ($rootScope.jumpDecimal === undefined) {
                                ...
                                bufferNumber =
                                    $rootScope.jumpDecimal * 10 + (code - 48);
                                $scope.$parent.setActiveBuffer(
                                    bufferNumber,
                                    "$jumpKey"
                                );
                    ...
                    if (
                        ...
                        if (
                            ($scope.$parent.search.length ||
                                $scope.$parent.onlyUnread) &&
                            $scope.$parent.filteredBuffers.length
                        ) {
                            filteredBufferNum =
                                $scope.$parent.filteredBuffers[bufferNumber];
                            if (filteredBufferNum !== undefined) {
                                activeBufferId = [
                                    filteredBufferNum.number,
                                    filteredBufferNum.id,
                                ];
                            }
                        } else {
                    ...
                    if (
                        $event.altKey &&
                        (code === 60 || code === 226 || key === "IntlBackslash")
                    ) {
                    ...
                    if (code === 27) {
                        ...
                        for (
                            var modalId = 0;
                            modalId < modals.length;
                            modalId++
                        ) {
                            if (
                                modals[modalId].getAttribute("data-state") ===
                                "visible"
                            ) {
                                modals[modalId].setAttribute(
                                    "data-state",
                                    "hidden"
                                );
                                return true;
                        ...
                        if (
                            typeof $scope.lastEscape !== "undefined" &&
                            Date.now() - $scope.lastEscape <= 500
                        ) {
                            // Double-tap
                    ...
                    if (
                        ...
                        $scope.command = models
                            .getActiveBuffer()
                            .getHistoryUp($scope.command);
                    ...
                    if (
                        ...
                        if ($scope.command) {
                            caretPos = inputNode.selectionStart;
                            if (
                                $scope.command.slice(caretPos).indexOf("\n") !==
                                -1
                            ) {
                                return false;
                            }
                    ...
                    if (
                        ...
                        for (i = lines.length - 1; i >= 0; i--) {
                            if (
                                lines[i].offsetTop - bufferlines.scrollTop <
                                bufferlines.clientHeight / 2
                            ) {
                    ...
                    if (
                        ...
                        if (code == 65) {
                            ...
                            var trimmedValue = $scope.command.slice(
                                0,
                                caretPos
                            );
                            var lastSpace =
                                trimmedValue
                                    ...
                                    .lastIndexOf(" ") + 1;
                            $scope.command =
                                $scope.command.slice(0, lastSpace) +
                                $scope.command.slice(
                                    caretPos,
                                    $scope.command.length
                                );
